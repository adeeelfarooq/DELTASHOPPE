            ...
            <div className="pin-box !ps-0 !flex !flex-row !flex-nowrap !justify-center !items-center !gap-3 md:!gap-5 !px-2 md:!px-4 !w-full">
                {
                    ...
                    teamData.slice(0, 6).map((card, index) => (
                        <div 
                            ...
                            className={`vd-card group overflow-hidden cursor-pointer aspect-[9/16] h-auto !relative !m-0 !w-[35vw] md:!w-[15vw] lg:!w-[14.5vw] xl:!w-[14vw] !border-white shadow-2xl`}
                            style={{ willChange: 'transform' }} 
                        >
                           ...
                           {/* 🟢 LinkedIn Icon Top Right pe */}
                           <a 
                               href={card.linkedin || "#"} 
                               target="_blank" 
                               rel="noreferrer"
                               className="absolute top-2 right-2 md:top-3 md:right-3 bg-black/60 hover:bg-[#ba1c3c] backdrop-blur-sm p-1.5 md:p-2 rounded-full opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-[-10px] group-hover:translate-y-0 z-30"
                           >
                               <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" viewBox="0 0 24 24" className="text-white">
                                   <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
                               </svg>
                           </a>
                           
                           {/* 🟢 Data Reveal Box */}
                           <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-black via-black/80 to-transparent pt-16 pb-4 px-2 md:px-3 flex flex-col justify-end">
                                ...
                                <div className="translate-y-4 group-hover:translate-y-0 transition-transform duration-500 ease-in-out flex flex-col items-center">
                                    ...
                                    <div className="grid grid-rows-[0fr] group-hover:grid-rows-[1fr] transition-all duration-700 ease-in-out w-full text-center">
                                        <div className="overflow-hidden opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-150 pt-2">
                                            <p className="text-[#ba1c3c] font-bold text-[9px] md:text-[11px] xl:text-xs mb-1.5 uppercase tracking-wider">
                                                {card.role}
                                            </p>
                                            <p className="text-gray-200 text-[8px] md:text-[9px] xl:text-[11px] font-paragraph leading-snug xl:leading-relaxed">
                                                {card.bio}
                                            </p>
                                        </div>
                                    </div>
